...
def tfidf(term, document, documents)
  "the tfdif output for #{term.upcase} in #{document[0, 50]} is #{tf(term, document) * idf(term, documents)}"
...
def tf(term, document)
  # p "the tf method is looking for #{term.upcase} in #{document[0, 20]}"
...
def idf(term, documents)
  # p "the idf method was called looking for #{term.upcase}"
  number_of_documents = documents.length
  number_of_documents_with_term = documents.count { |d| tf(term, d) > 0 }
  number_of_documents_with_term

  Math.log(number_of_documents.fdiv(number_of_documents_with_term)) unless number_of_documents_with_term == 0
  0 if number_of_documents_with_term == 0
end
