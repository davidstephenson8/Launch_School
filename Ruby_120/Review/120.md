...
## instance variables
These are vairables that exist as long as the object exists. They are created when the object is instantiated and are part of the state of that object.  
### scope and how inheritance affects scope
Instance variables are accessable for that specific instance, and can be made avaiable outside of the instance via getter methods. Instance variables are able to be inherited by subclasses when they're defined in superclasses. For example
...
These variables can be modified by any subclass, so they're a little bit dangerous. 
...
The only way to compare objects in Ruby is to have a method explicitly defined that gives Ruby the rules for how to compare the objects. In general, if a `<=>` method is defined, Ruby can work out greater than, less than and equality evaluations. If you can crib from another class's `<=>` great, otherwise you need to return -1 0 and 1 at the right times. 
