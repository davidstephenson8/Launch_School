...
- [numbers, including NaN and Infinity](#numbers-including-nan-and-infinity)
  ...
  - [NaN](#nan)
  - [Infinity](#infinity)
- [strings properties](#strings-properties)
...
## string methods
- ``String#include?`` returns true if argument passed to method is found in string. Similar methods available for hashes and arrays. 
...
- ``Array#include?`` returns true if value is contained in array 
- ``Enumerable#any?`` returns true if any of the values in the array return true when passed to block 
# hash properties
- a collection of key-value pairs. Order is loosely maintained in Ruby, but hashes with different orders to key-value pairs are considered equivalent
- values are returned by passing a key to the ``Hash[]`` method.
## hash methods
- ``Hash#include?`` returns true if argument is a key in the hash the method is called on. 
  - More idiomatic to use aliases ``#key?`` or ``#has_key?`` because the intention is more clear
- ``Hash#keys`` returns an array with all of the keys in the hash
- ``Enumerable#any?`` returns true if any of the values in the array return true when passed to block. Takes two block parameters
- ``Enumerable#all?`` returns true if all iterations of the block return a truthy value
# operators
